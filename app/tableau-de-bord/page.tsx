...
const sauvegarderProfil = async () => {
  try {
    ...
    // ✅ UTILISER L'URL EXISTANTE, PAS LA PRÉVISUALISATION
    const dataAEnvoyer = {
      id: utilisateur.id,
...
const gererChangementPhoto = async (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  try {
    ...
    if (resultat.success && resultat.avatar_url) {
      ...
      console.log('✅ Avatar uploadé:', avatarUrlWithTimestamp);
...
const supprimerPhoto = async () => {
  ...
  try {
    ...
    if (resultat.success) {
      ...
      setAlerteSucces('Photo supprimée !');
    }
  } catch (error) {
    console.error('Erreur:', error);
  }
                ...
                <img 
                  ...
                  onError={(e) => {
                    ...
                    if (parent) {
                      ...
                      span.textContent = `${utilisateur.prenom?.[0] || ''}${utilisateur.nom?.[0] || ''}`;
